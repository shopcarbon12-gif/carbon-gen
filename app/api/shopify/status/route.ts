...
async function discoverAutoConnectedShop() {
  const data = await listShopifyTokenRecords(20);
  ...
  for (const row of data) {
    ...
    const token = String((row as any)?.accessToken || "").trim();
    ...
    return {
      shop,
      installedAt: String((row as any)?.installedAt || "") || null,
    };
  }
...
export async function GET(req: NextRequest) {
  try {
    ...
    const dbRow = await getShopifyTokenRecord(shop);
    const dbToken = String(dbRow?.accessToken || "").trim();
    ...
    for (const candidate of candidates) {
      ...
      if (probe.ok) {
        return NextResponse.json({
          connected: true,
          shop,
          installedAt: candidate.source === "db" ? dbRow?.installedAt || null : null,
    ...
    return NextResponse.json({
      ...
      shop: null,
      reason: "error",
      error: e?.message || "Shopify status check failed",
    });
  }
}
