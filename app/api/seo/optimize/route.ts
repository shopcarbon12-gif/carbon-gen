...
// Fields the AI generates (vendor, product type and image alt are intentionally
// excluded — vendor/type are preserved from Shopify; alt lives in the push section).
const GEN_FIELDS: SeoFieldKey[] = ["seoTitle", "metaDescription", "handle", "title", "bodyHtml", "tags"];
...
const SCHEMA = `{
  ...
  "proposed": {
    ...
    "handle": string,              // lowercase-hyphenated, include the focus keyword words, no stop words/underscores
    "title": string,               // 25-65 chars descriptive H1 including the focus keyword
  ...
  "rationale": { "seoTitle": string, "metaDescription": string, "handle": string, "title": string, "bodyHtml": string, "tags": string }
}`;

function buildGenInstruction(context: ProductContext, imageIds: string[], useVision: boolean) {
  ...
  return [
    ...
    "",
    useVision && imageIds.length ? `Photos are attached in order.` : "",
    "",
...
function buildProposed(parsed: any, current: SeoFields, focusKeyword: string, secondaryKeywords: string[]): SeoFields {
  ...
  return {
    title: String(p.title || current.title || "").trim(),
    seoTitle: String(p.seoTitle || "").trim(),
    metaDescription: String(p.metaDescription || "").trim(),
    handle: String(p.handle || current.handle || "").trim().toLowerCase(),
    bodyHtml: String(p.bodyHtml || "").trim(),
    tags: asStringArray(p.tags, 20),
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!parsed || !parsed.proposed) return NextResponse.json({ error: "Optimizer returned no usable result. Please retry." }, { status: 502 });
    ...
    // ---- Refinement: push every generated field to the 95-100 range ----
    for (let pass = 0; pass < MAX_REPAIRS; pass += 1) {
      const weak = GEN_FIELDS.filter((f) => (proposedScorecard.fields[f]?.score ?? 0) < TARGET_SCORE);
      ...
      const repairPrompt = [
        ...
        "",
        "Targets: seoTitle 40-60 chars w/ keyword; metaDescription 130-155 chars w/ keyword + CTA; handle lowercase-hyphenated w/ keyword; title 25-65 w/ keyword; bodyHtml HTML (<p> + <ul> 4-6 <li> + <p>), 450-850 chars text, keyword present; tags 7-12 lowercase deduped.",
