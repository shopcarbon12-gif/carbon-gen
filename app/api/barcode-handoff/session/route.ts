...
import { createBarcodeHandoffSession } from "@/lib/barcode-handoff-store";
...
export async function POST(request: Request) {
  const session = createBarcodeHandoffSession();
  const origin = buildRequestOrigin(request);
  const scanUrl = `${origin}/barcode-scan/${session.id}`;
