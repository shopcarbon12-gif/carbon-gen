...
export type GeminiWorkspaceMode = "all" | "images" | "ops-seo";

type GeminiWorkspaceProps = {
  mode?: GeminiWorkspaceMode;
};

export default function GeminiWorkspace({ mode = "all" }: GeminiWorkspaceProps) {
  ...
  // Pose/face variation strength + rotating seed (see lib/poseVariation.ts).
  ...
  const [generateGeminiResponse, setGenerateGeminiResponse] = useState<string | null>(null);
  ...
  const runPoseScan = useCallback(
    async (opts?: { genders?: Array<"male" | "female"> }) => {
      ...
      try {
        ...
        const resp = await fetch("/api/gemini/item-scan", {
          method: "POST",
  ...
  function formatGenerateDebugPayload(json: any, panelNumber: number) {
    const source = json?.geminiRaw ?? json?.openaiRaw ?? json?.error ?? json;
  ...
  async function persistItemReferences(options?: { silentSuccess?: boolean }) {
    ...
    const devicePreviewDataUrls = await Promise.all(
      files.map((file) => fileToDataUrl(file).catch(() => ""))
    );
    ...
    const uploadedDeviceSelections: SelectedCatalogImage[] = uploadedUrls.map((uploadedUrl, idx) => {
      ...
      const localPreviewUrl = String(devicePreviewDataUrls[idx] || "").trim() || uploadedUrl;
      return {
    ...
    if (uploadedDeviceSelections.length) {
      setSelectedCatalogImages((prev) => {
        ...
        const skipped = uploadedDeviceSelections.filter((img) =>
          existingUploaded.has(String(img.uploadedUrl || "").trim())
        );
        // Nothing to release for data URLs.
        void skipped;
  ...
  async function loadCatalogImages(options?: {
    ...
    try {
      ...
      // Keep mobile browse compact and swipeable.
      for (let guard = 0; guard < 25; guard += 1) {
        const mobileCatalogPageSize =
          typeof window !== "undefined" && window.matchMedia("(max-width: 900px)").matches
            ? 2
            : CATALOG_PAGE_SIZE;
        const params = new URLSearchParams({
          shop: shopValue,
          first: String(mobileCatalogPageSize),
        });
  ...
  async function generateAltForPushImage(imageId: string) {
    ...
    try {
      ...
      const resp = await fetch("/api/gemini/image-alt", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          imageUrl: altSource,
      ...
      const json = await parseJsonResponse(resp, "/api/gemini/image-alt");
      if (!resp.ok) {
...
function buildMasterPanelPromptLegacy(args: {
    ...
    const lines: string[] = [
      `Generate a professional ecommerce fashion catalog panel image (Panel ${args.panelNumber}: ${args.panelLabel}).`,
      "",
      `Canvas: 4096×2731 (3:2). Split into LEFT frame (2048×2731) and RIGHT frame (2048×2731) with a thin white divider.`,
      "",
      `Model: ${args.modelName} (${args.modelGender}), adult 25+.`,
      `Item type: ${promptItemType || args.itemType}.`,
      "",
      "PERSON IDENTITY: Copy the exact person from the MODEL reference images — same face shape, eye shape/spacing, nose, lips, jawline, skin tone, hair color/style, and body. Do NOT use identity from item reference images.",
      "GARMENT: Copy the exact garment from the ITEM reference images — same color, design, pattern, fabric, logos, prints, and construction. Do NOT invent or change any garment details.",
      ...(isSwimwearItemType(args.itemType)
        ? ["This is swimwear — professional catalog presentation, neutral posture, non-suggestive."]
        : []),
      "",
      `LEFT FRAME — Pose ${args.poseA}:`,
      poseABlock,
      "",
      `RIGHT FRAME — Pose ${args.poseB}:`,
      poseBBlock,
      "",
      "Rules:",
      "- Photorealistic human anatomy and skin. No CGI or mannequin look.",
      "- Full-body poses: show complete head-to-shoes with white margin. No cropping of head, hair, or feet.",
      ...(swimwearActive
        ? ["- Footwear: flip-flops, sandals, or naturally bare feet for swimwear."]
        : ["- Footwear: visible shoes on all full-body poses. No barefoot."]),
      "- Both frames show the same outfit. No outfit swaps between left and right.",
      "- If item refs show a back design, reproduce it exactly. If not, keep back solid/clean.",
      "- Hands visible, not in pockets.",
      "- Non-sexual, professional storefront-safe presentation.",
      "- Center each pose in its half so a 3:4 crop keeps the subject intact.",
    ];

    if (styleInstructions) {
      lines.push("", "Styling instructions:", styleInstructions);
    }
    if (regenNotes) {
      lines.push("", "Regeneration feedback:", regenNotes);
    }
    if (args.poseSafetySuggestions && args.poseSafetySuggestions.length) {
      lines.push("", "Pose safety modifications:", ...args.poseSafetySuggestions);
    }
...
function buildMasterPanelPrompt(
    ...
    try {
      ...
      const generateOnePanel = async (panelNumber: number) => {
        try {
          ...
          const requestOnce = async (
            ...
            if (!resp.ok) {
              const geminiRelated =
                Boolean(json?.openaiRaw || json?.geminiRaw) ||
                /gemini|openai|policy|safety|content/i.test(String(json?.error || ""));
              if (geminiRelated) {
      ...
      if (useAllSelected) {
        ...
        for (let qi = 0; qi < queue.length; qi++) {
          const panelNumber = queue[qi];
          setStatus(`${actionWord} panel ${panelNumber} (${qi + 1}/${queue.length})...`);
          try {
            const result = await generateOnePanel(panelNumber);
            succeeded[result.panelNumber] = result.b64;
            setGeneratedPanels((prev) => ({ ...prev, [result.panelNumber]: result.b64 }));
            ...
            setPanelFailReasons((prev) => ({ ...prev, [panelNumber]: err?.message || "Generation failed." }));
          }
        }
        ...
        if (failed.length) {
          ...
          const failedPanels = uniqueSortedPanels(
            failed.map((entry) => entry.panelNumber).filter((v) => Number.isFinite(v))
          );
          ...
          if (!succeededPanels.length) throw new Error(uniqueFailed.join(" | "));
        }

        setStatus(`${actionWord} completed: panel(s) ${succeededPanels.join(", ")}.`);
      } else {
  ...
  function appendGenerateRawResponse(payload: string) {
    ...
    if (!clean) return;
    setGenerateGeminiResponse((prev) => {
  ...
  async function sendDialogMessage() {
    ...
    setError(null);
    try {
      const resp = await fetch("/api/gemini/dialog", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          messages: next,
          contextError: generateGeminiResponse || "",
      ...
      const json = await parseJsonResponse(resp);
      if (!resp.ok) {
        const exact = json?.geminiRaw ?? json?.openaiRaw ?? json?.details ?? json;
        const exactText =
          typeof exact === "string" ? exact : JSON.stringify(exact, null, 2);
        setGenerateGeminiResponse(exactText);
        throw new Error(json?.error || "Gemini dialog failed");
      }
      const reply = typeof json?.reply === "string" ? json.reply : "";
      setDialogMessages((prev) => [...prev, { role: "assistant", content: reply || "(No response text)" }]);
    } catch (e: any) {
      setError(e?.message || "Gemini dialog failed");
    } finally {
  ...
  async function copyErrorPayload() {
    try {
      const payload = generateGeminiResponse || "";
      if (!payload.trim()) {
      ...
      setError(null);
      setStatus("Copied Gemini error payload to clipboard.");
    } catch {
  ...
  function onStatusBarCopyRawResponse() {
    if (!hasRawGeminiResponse) return;
  ...
  return (
    ...
    >
      ...
      <section
        ...
        aria-live="polite"
        aria-atomic="true"
        role={hasRawGeminiResponse ? "button" : undefined}
        tabIndex={hasRawGeminiResponse ? 0 : undefined}
        onClick={hasRawGeminiResponse ? onStatusBarCopyRawResponse : undefined}
        onKeyDown={
      ...
      <main className="grid">
        ...
        <section id="publish-section" className="card">
          ...
          <div className="model-registry-header">
            ...
            <div className="model-registry-header-actions">
              ...
              <button
                ...
                title={modelRegistryCollapsed ? "Expand section" : "Collapse section"}
              >
          ...
          {!modelRegistryCollapsed && previousUploadsVisible ? (
            <div className="card">
              <div className="card-title">Previous Uploads</div>
              <div className="row previous-upload-filters-row">
                <select
                  value={previousSort}
                  onChange={(e) =>
          ...
          {!modelRegistryCollapsed ? (
            <div className="row model-save-row" style={{ justifyContent: "center", alignItems: "center" }}>
              <button
              ...
              <button className="ghost-btn danger center-reset-btn" type="button" onClick={resetModels}>
                Reset all models
              </button>
            </div>
          ) : null}
        ...
        <section className="card">
          ...
          <div className="section-header">
            ...
            <button
              ...
              className="ghost-btn icon-toggle-btn"
              type="button"
              onClick={() => setItemRefsCollapsed((p) => !p)}
              title={itemRefsCollapsed ? "Expand section" : "Collapse section"}
            >
              {itemRefsCollapsed ? "⤢" : "⤡"}
            </button>
          </div>
          ...
          />
          <div className="picker-row item-picker-row">
          ...
          <div className="catalog-wrap">
              ...
              {shop.trim() && !catalogSearched && (
                <div className="muted centered">
                  Search by product name, or leave search empty to browse catalog pages.
                </div>
              )}
                  ...
                  {visibleCatalogProducts.map((product) => (
                    <div className="catalog-product" key={product.id}>
                      <div className="catalog-title">
                        {product.title} | Barcode: {formatProductBarcodes(product)}
                      </div>
                      <div className="preview-grid item-catalog-grid mobile-carousel">
        ...
        <section className="card">
          ...
          <div className="section-header">
            ...
            <button
              className="ghost-btn icon-toggle-btn"
              type="button"
              onClick={() => setGenerateCollapsed((p) => !p)}
              title={generateCollapsed ? "Expand section" : "Collapse section"}
            >
              {generateCollapsed ? "⤢" : "⤡"}
            </button>
          </div>
          ...
          <div className="panel-row">
            {panels.map((panel) => {
              ...
              const compactPanelLabel = panelLabel.replace(/\s*\(.*?\)\s*/g, "").trim();
              ...
              return (
                ...
                >
                  {unavailableForDress ? `Panel ${panel.id} (Not available for dress)` : compactPanelLabel}
                </button>
              );
            })}
          </div>
          ...
          <div className="generation-actions-layout">
            ...
            <div className="generation-comments-wrap">
              <textarea
                ...
                placeholder='Regeneration comments (example: "make fit less oversized", "match logo placement exactly").'
              />
              <button
                className="btn ghost"
          ...
          <div className="row split-external-row" style={{ justifyContent: "center" }}>
        ...
        <section className="card">
          ...
          <div className="section-header">
            ...
            <button
              className="ghost-btn icon-toggle-btn"
              type="button"
              onClick={() => setResultsCollapsed((p) => !p)}
              title={resultsCollapsed ? "Expand section" : "Collapse section"}
            >
              {resultsCollapsed ? "⤢" : "⤡"}
            </button>
          </div>
          {!resultsCollapsed ? (
          <>
          <div className="row final-download-row">
              <button
                className="btn ghost"
                type="button"
            ...
            <div className="row final-save-row">
              <button
                className="btn primary"
                type="button"
            ...
            <div className="row final-seo-row">
              <button
                className="btn ghost"
                type="button"
              ...
              >
                {splitSendingToPush ? "Sending Results..." : "Use Results In SEO Manager"}
              </button>
            </div>
            <div
      ...
      <style jsx>{`
        ...
        @media (min-width: 901px) {
          .model-pill .model-info {
            flex: 1 1 auto;
          }
          .model-pill .model-remove {
            margin-left: auto;
          }
        }
        @media (max-width: 900px) {
          ...
          .panel-preview-grid > .panel-preview-card {
        ...
        @media (max-width: 640px) {
          ...
          .model-registry-header-actions .danger {
            order: 1;
          }
          .barcode-action-row > .btn,
          .barcode-action-row > .ghost-btn,
